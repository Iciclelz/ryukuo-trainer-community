...
namespace Ryukuo_Trainer_Community
{
 ...
 bool unicode, bool casesensitive, bool percentage, bool compareToSavedScan, [MarshalAs(UnmanagedType.BStr)] string savedscanname);
        ...
        {
            libInst = LoadLibrary("ryukuo-trainer-community.dll");
            if (libInst != IntPtr.Zero)
            {
                loadFunctions();
            }
            else
                MessageBox.Show("Cannot load the memory editing module. None of the hacks will work. Please restart the trainer.", "Ryukuo Trainer Community", MessageBoxButton.OK, MessageBoxImage.Error);
